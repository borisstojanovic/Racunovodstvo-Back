...
public class BilansRestController {
    ...
    public ResponseEntity<?> getBrutoBilans(@RequestParam BilansRequest bilansRequest){
        if(bilansService.findBrutoBilans(bilansRequest).isEmpty()){
            return ResponseEntity.notFound().build();
        }else{
            return ResponseEntity.ok(bilansService.findBrutoBilans(bilansRequest));
        }
    }
}
